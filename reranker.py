...
from rapidfuzz.string_metric import normalized_levenshtein as nlev  # 선택: 제목/키워드 근접도
...
def rerank(contexts: List[Dict[str, Any]], hints: Dict[str, Any], query: str,
    ...
    if vmin == vmax:
        # 리트리버 점수가 없을 때: 앞쪽(원 리트리버 상위)이 조금 더 유리하도록 폴백
        n = len(vecs)
    ...
    for i in range(len(texts)):
        s = (W["vec"]*vec_norm[i] + W["bm25"]*bm_norm[i] +
             W["meta"]*meta[i]     + W["ver"]*ver[i]     +
             W["uri"]*uri_hit[i])
        scores.append(s)
    ...
    while cand and len(selected) < k:
        ...
        def sim(a,b):  # 매우 단순한 중복 억제: 제목/내용 nlev
        ...
        selected.append(mmr_idx); cand.remove(mmr_idx)
